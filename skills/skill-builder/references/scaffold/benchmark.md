...
- The skill runs repeatable benchmark campaigns and needs `projects/<name>/v<N>/` history
...
└── projects/
    └── <project-name>/
        ...
        │       ├── results.jsonl
        │       ├── report.md
        │       └── worktrees/    # optional; one isolated lane per candidate
        ├── v2/
        │   ├── input.md
        │   └── output/
        └── current.md
...
git worktree add -b bench/<project>-<lane> .worktrees/<project>-<lane> main
```
...
- If multiple OSS candidates exist, compare at least 2 before freezing the benchmark set
...
```bash
node scripts/bench.mjs --project <name> --preset <id> --parallel <n> --report-md
```

Expected output:
- `projects/<name>/v<N>/input.md`
- `projects/<name>/v<N>/output/lanes/*.json`
- `projects/<name>/v<N>/output/results.jsonl`
- `projects/<name>/v<N>/output/report.md`
- `projects/<name>/current.md`
````
