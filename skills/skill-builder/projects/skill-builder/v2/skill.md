...
Full templates → `references/scaffold.md` · Archetype patterns → `references/archetypes.md`

### 2.0 Archetype selection

Before choosing a directory structure, pick the archetype that matches the skill's purpose:

| Archetype | Pick when… |
|---|---|
| *(none — Pattern A/B/C/D)* | Simple workflow, no special output shape |
| **E — Project-versioned** | Skill produces iterated outputs for a named project (`v1/`, `v2/` …) |
| **F — Meta-orchestrator** | Skill compares libraries/options via multiple sub-skills and recommends |
| **G — Skill sandbox** | You want to validate the skill itself with fixtures, diffs, and insight extraction |

Archetypes can be combined (e.g. F + G: a meta-orchestrator that also has a sandbox).
Full layouts and templates → `references/archetypes.md`
